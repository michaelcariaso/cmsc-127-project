...
import AddFood from "./foodItem/AddFood";
import EditFood from "./foodItem/EditFood";

export default function FoodItem({ establishment_id, setInv }) {
  const [foodItems, setFoodItems] = useState([]);
  const [itemName, setName] = useState("");
  const [itemPrice, setPrice] = useState(0);
  const [itemType, setType] = useState("");
  const [establishmentId, setEstablishmentId] = useState(establishment_id);
  ...
  function fetchFoodItems() {
    fetch(
      `http://localhost:4000/foods?establishment_id=${establishmentId}&order=${order}`
    )
