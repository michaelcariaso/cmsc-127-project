...
async function searchFoodParams(req, res) {
  ...
  const buildQuery = (food_type, min_price, max_price) => {
    ...
    let queryString = `SELECT * FROM food_item WHERE ${conditions.join(
      " AND "
    )}`;
  ...
  try {
    const [result] = await pool.query(
      buildQuery(food_type, min_price, max_price)
    );
    res.status(200).json(result);
  } catch (error) {
    console.error("Error finding food item:", error);
...
export async function updateFoodItem(req, res) {
  const { item_name, item_price, food_type, item_id } = req.body;
  try {
    const result = await pool.query(
      `UPDATE food_item SET item_name = ?, item_price = ?, food_type = ? WHERE item_id = ?;`,
      [item_name, item_price, food_type, item_id]
    );
    res.status(200).json(result);
  } catch (error) {
...
export async function findFood(req, res) {
  ...
  try {
    ...
    console.error("Error finding food item:", error);
    res.status(500).json({ error: "Failed to find food item" });
    throw error;
  }
}
